...
def check_lambdas(lambda_min, lambda_max, y, order = 2, tol = 1e-8):
...
#             break
    ...
    out = tikhonov(y, lambd = lambda_min, order = order)
    residual_min, reg_residual_max = out[1]
    out = tikhonov(y, lambd = lambda_max, order = order)
    residual_max, reg_residual_min = out[1]
...
def tikhonov(y, lambd = 0, order = 2, verbose = False):
    ...
    # Solve l2 filtering problem.
    x = cp.Variable(shape=n)
    obj = cp.Minimize(0.5 * cp.sum_squares(y - x) + lambd * cp.norm(D@x, 2) )
    ...
    residual = norm(y - x)
    reg_residual = norm(D * x, ord = 2)
    
    return [x, (residual, reg_residual)]

def lcurve_corner(y, order = 2, lambda_max = 1e10, lambda_min = 1e-10, epsilon = 0.001, max_iter = 50, normalize = False, verbose = False):
    ...
    while (gap >= epsilon) and (itr <= max_iter):
        ...
        if itr == 0:
            ...
            for s in range(4):
                ...
                out = tikhonov(y, lambd = current_lambda, order = order)
                ress[s], regs[s] = out[1]
        ...
        while C3 < 0:
            ...
            out = tikhonov(y, lambd = lambda_vec[1], order = order)
            res, reg = out[1] 
        ...
        if C2 > C3:
            ...
            out = tikhonov(y, lambd = lambda_vec[2], order = order)
            res, reg = out[1] 
...
def full_lcurve(y, order = 2, lambda_max = 1e10, lambda_min = 1e-10, n_lambdas = 100, normalize = False, plot_lc = False):
    ...
    if normalize:
        ...
        if plot_lc:
            ...
            plt.ylabel(r'$||D^{0}\beta||_2$'.format(order))
