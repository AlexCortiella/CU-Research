...
def check_lambdas(lambda_min, lambda_max, y, x, order = 3, tol = 1e-8):
    
#     #Make sure that the solver does not fail for the range of lambdas 
#     while True:
#         try:
#             out = trend_filter_der(y, x, lambd = lambda_min, order = order)
#             beta_min = out[0]
...
#     while True:
#         try:
#             out = trend_filter_der(y, x, lambd = lambda_max, order = order)
#             beta_max = out[0]
...
def trend_filter_der(y, x, lambd = 0, order = 3, verbose = False):
    ...
    dy = dxdt(y, x, kind="trend_filtered", order=order, alpha=lambd, max_iter = 10000)
    u = 0.5*(dy[0:-1] + dy[1:])
...
def lcurve_corner(y, x, order = 3, lambda_max = 1e10, lambda_min = 1e-10, epsilon = 0.001, max_iter = 50, normalize = False, verbose = False):
    ...
    while (gap >= epsilon) and (itr <= max_iter):
        ...
        if itr == 0:
            ...
            for s in range(4):
                ...
                out = trend_filter_der(y, x, lambd = current_lambda, order = order)
                ress[s], regs[s] = out[1]
        ...
        while C3 < 0:
            ...
            out = trend_filter_der(y, x, lambd = lambda_vec[1], order = order)
            res, reg = out[1] 
        ...
        if C2 > C3:
            ...
            out = trend_filter_der(y, x, lambd = lambda_vec[2], order = order)
            res, reg = out[1] 
...
def full_lcurve(y, x, order = 3, lambda_max = 1e10, lambda_min = 1e-10, n_lambdas = 100, normalize = False, plot_lc = False):
    ...
    lambda_max, lambda_min = check_lambdas(lambda_min, lambda_max, y, x, order, tol = 1e-8)
    ...
    for i, lambd in enumerate(lambdas):
        ...
        out = trend_filter_der(y, x, lambd, order = 3)
        dy[:,i] = out[0]
...
def gcv(y, x, order = 3, lambda_min = 1e-10, lambda_max = 1e10, n_lambdas = 100, plot_lc = False):
...
#     lambda_max, lambda_min = check_lambdas(lambda_min, lambda_max, y, x)
    ...
    for i, lambd in enumerate(lambdas):
        ...
        tf = trend_filter_der(y, x, lambd = lambd, order = order)
        gcv_lambda[i] = tf[2][0]
    ...
    #Solve for optimal lambda
    tf_opt = trend_filter_der(y, x, lambd = lamb_opt, order = order)
    dy_tf = tf_opt[0]
    ...
    return [dy_tf, (lambdas, gcv_lambda)]