...
def tikhonov(y, lambd = 0, verbose = False):
    ...
    #Generate the difference operator
    D = diff_mat(m,2)
    ...
    GCV = (m * norm(u - y)**2) / (m - df) ** 2
    
    return [u, (residual, reg_residual), (GCV, df)]

def lcurve_corner(y, lambda_max = 1e10, lambda_min = 1e-10, epsilon = 0.001, max_iter = 50, normalize = False, verbose = False):
    ...
    while (gap >= epsilon) and (itr <= max_iter):
        ...
        if itr == 0:
            ...
            for s in range(4):
                ...
                out = tikhonov(y, lambd = current_lambda)
                ress[s], regs[s] = out[1]
        ...
        while C3 < 0:
            ...
            out = tikhonov(y, lambd = lambda_vec[1])
            res, reg = out[1] 
        ...
        if C2 > C3:
            ...
            out = tikhonov(y, lambd = lambda_vec[2])
            res, reg = out[1] 
    ...
    return [y, current_lambda, lambda_itr, (lc_res, lc_reg)]

def full_lcurve(y, lambda_max = 1e10, lambda_min = 1e-10, n_lambdas = 100, normalize = False, plot_lc = False):
    
    residual_lc = np.zeros(n_lambdas)
    reg_residual_lc = np.zeros(n_lambdas)
    n = len(y)
    y_tik_path = np.zeros((n, n_lambdas))
    
    #Check range of lambdas and compute normalization coefficients
    #lambda_max, lambda_min = check_lambdas(lambda_min, lambda_max, y, tol = 1e-8)
