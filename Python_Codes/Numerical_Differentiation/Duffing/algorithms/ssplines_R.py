...
def gcv(y, x, lambda_min = 1e-10, lambda_max = 1e10, n_lambdas = 100, plot_lc = False):
    ...
    m_samples = len(y)
    y_ss_path = np.zeros((m_samples,n_lambdas))
...
#     lambda_max, lambda_min = check_lambdas(lambda_min, lambda_max, y, x)
    ...
    for i, lambd in enumerate(lambdas):
        ...
        #Run ssplines with current lambda
        sspline = ssplines(y, x, lambd = lambd)
        gcv_lambda[i] = sspline[2][0]
        y_ss_path[:,i] = sspline[0][0]
    ...
    #Solve for optimal lambda
    sspline_opt = ssplines(y, x, lambd = lamb_opt)
    y_ss, dy_ss = sspline_opt[0]
    ...
    return [y_ss, dy_ss, (lambdas, gcv_lambda), y_ss_path]