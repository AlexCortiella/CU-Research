...
# def lcurve_corner(X, y, lambda_min = 1e-10, lambda_max = 1e10, epsilon = 0.001, max_iter = 50, normalize = False, verbose = True):
...
#     lambda_max, lambda_min, normalization_coefs = check_lambdas(X, y, lambda_min, lambda_max)
...
#                 residuals[s] = norm(np.dot(X, beta) - y)
#                 reg_residuals[s] = norm(beta_tilde, ord = 1)
...
def lcurve_corner(X, y, w = None, lambda_min = 1e10, lambda_max = 1e-10, epsilon = 0.001, max_iter = 50, tol = 1e-8, normalize = False, verbose = False):
...
def full_lcurve(X, y, w = None, lambda_min = 1e-10, lambda_max = 1e10, n_lambdas = 100, tol = 1e-8, normalize = False, plot_lc = False):
    ...
    #Check the range of lambdas and compute normalization coefficients
    lambda_max, lambda_min = check_lambdas(lambda_min, lambda_max, X, y, tol)
