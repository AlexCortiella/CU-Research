...
def gcv(y, x, order = 3):
    ...
    n_lambdas = len(df)
    gcv_func = np.zeros(n_lambdas)
    
    m_samples = len(y)
    
    for i in range(n_lambdas):
        gcv_func[i] = 1/m_samples * np.sum(((r_y - y_tf[:,i])/(1 - df[i]/m_samples)) ** 2)
        
    min_idx = np.argmin(gcv_func)
        
    plt.loglog(lambdas, gcv_func)
    plt.loglog(lambdas[min_idx], gcv_func[min_idx],'ro')
    plt.ylabel(r'$GCV(\lambda)$')
    plt.xlabel(r'$\lambda$')
    
    return [y_tf[:,min_idx], (gcv_func, lambdas)]