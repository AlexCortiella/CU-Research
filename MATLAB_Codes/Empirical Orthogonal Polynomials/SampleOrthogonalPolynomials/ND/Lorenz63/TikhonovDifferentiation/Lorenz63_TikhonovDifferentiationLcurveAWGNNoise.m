...
% x1noisy = x1true + sigma1(j)*randn(Nsamples,1);
x1noisy = x1true + sigma1*randn(Nsamples,1);
% x1noisy = x1true + NLx*norm(x1true)/100*randn(Nsamples,1);
% X2 state
% x2noisy = x2true.*(1+etax*randn(Nsamples,1));
% x2noisy = x2true + sigma2*randn(Nsamples,1);
% x2noisy = x2true + sigma2(j)*randn(Nsamples,1);
x2noisy = x2true + sigma2*randn(Nsamples,1);

% x2noisy = x2true + NLx*norm(x2true)/100*randn(Nsamples,1);
% X3 state
% x3noisy = x3true.*(1+etax*randn(Nsamples,1));
% x3noisy = x3true + sigma3*randn(Nsamples,1);
% x3noisy = x3true + sigma3(j)*randn(Nsamples,1);
x3noisy = x3true + sigma3*randn(Nsamples,1);
...
save('Lorenz63TikhonovNumDiff200','errordxLcnoise','residualLcnoise','regularizerLcnoise','resCorner','regCorner','Lorenz63Data')

%Save data structure
% save('Lorenz63DataStruc400','Lorenz63Data')
