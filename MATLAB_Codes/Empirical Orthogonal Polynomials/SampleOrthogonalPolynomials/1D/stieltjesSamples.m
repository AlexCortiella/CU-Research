...
%
function ab=stieltjesSamples(d,x)
tiny=10*realmin;
huge=.1*realmax;
N = length(x);%Number of samples
xw = [x,ones(N,1)];%Weights ( in this case all are 1)
...
for k=1:d-1
  ...
  s1=1/N*xw(:,2)'*(p2.^2);% denominator of alpha_k --> inner product <pk,pk>_dL = int_support(pk(x)^2)dL where dL is the probability measure
  s2=1/N*xw(:,1)'*(xw(:,2).*(p2.^2));% numerator of alpha_k --> inner product <x*pk,pk>_dL = int_support(x*pk(x)^2)dL where dL is the probability measure  
