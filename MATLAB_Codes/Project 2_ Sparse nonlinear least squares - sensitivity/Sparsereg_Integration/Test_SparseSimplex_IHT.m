...
for iter = 1:Niter %iteration loop
    ...
    for s = 1:n %loop over states
        ...
        cond(Sps)
        gradLs = -1/m*ress'*Sps; %Gradient of L for state s
        
        Cps = 1/(2*m)*ress'*ress; %Cost function = Least squares term for state s
        Xis = Xis - alpha*gradLs';%alpha = 1/L is the step size

        [Xisort,isort]=sort(abs(Xis));%Sort the elements of Xis from small to large
        Xisnew=zeros(p,1);
        sps = sp(s);
        Xisnew(isort(p-sps+1:p))=Xis(isort(p-sps+1:p));%Keep only the sps largest elements of Xis (absolute value) and discard the other ones
        Xis=Xisnew;        
        options_s = odeset('RelTol',1e-10,'AbsTol',1e-10);
        [tps,xps] = ode45(@(t,x)integrateApprxDynamics(t,x,Xis,index_pc,x0),timeData,x0(s),options_s);
