...
%% NOISE MODELS
sigma1 = 0*0.01; %Noise level state 1
sigma2 = 0*0.01; %Noise level state 2
...
ress = xp(:,s) - xm(:,s);%residual vector for state s
Sps = squeeze(Sp(s,:,:))';
normGrad(1,s) = norm(Sps'*ress);
...
for iter = 1:Niter %iteration loop
    ...
    for s = 1:n %loop over states
        ...
        %% Compute residual and sensitivity for state s
        ress = xp(:,s) - xm(:,s);%residual vector for state s %residual vector for state s (residual = x_meas - x_pred)
        Sps = squeeze(Sp(s,:,:))'; %Sensitivity for state s
        %% Levenberg Marquadt algorithm
        %Compute LHS and RHS
%         LHS = Sps'*Sps + lambda(s)*diag(diag(Sps'*Sps));
        LHS = Sps'*Sps + lambda(s)*eye(p);
        ...
        options_s = odeset('RelTol',1e-10,'AbsTol',1e-10);
        [tps,xps_try] = ode45(@(t,x)integrateApprxDynamics(t,x,Xi_try,index_pc,x0),timeData,x0(s),options_s);
        ress_try = xps_try - xm(:,s);
        Grad_try = Sps'*ress_try;
...
subplot(2,4,4)
title('Norm of gradient vs iteration #')
semilogy(1:Niter+1,normGrad(:,1),'k')
xlabel('iteration #')
ylabel('Norm of gradient')
legend('state x_1')
subplot(2,4,8)
semilogy(1:Niter+1,normGrad(:,2),'k--')
xlabel('iteration #')
ylabel('Norm of gradient')
legend('state_x_2')